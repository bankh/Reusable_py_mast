...
class Maze():
    ...
    def __init__(self):
        # Disctionary to hold room_number, room_obj <key, value> pairs
        self._rooms = {}
...
# Interface (Specific to Builder Design Pattern)
# Pass included since overwrite is doable in python
class MazeBuilder():
    def __init__(self):
    ...
    def BuildDoor(self, roomFrom, roomTo):  #Place a door between the rooms
        pass                                # Empty can be overwritten
...
class MazeGame():
    ...
    def CreateMaze(self, builder): # Pass is a MazeBuilder
        builder.BuildMaze()
...
class StandardMazeBuilder(MazeBuilder):
    ...
    def BuildRoom(self, n):
        try: self._currentMaze.RoomNo(n)
            ...
            print(' Room {} does not exist'.format(n))
            room = Room(n)
            self._currentMaze.AddRoom(room)

            room.SetSide(Direction.North.value, Wall())  # All sides are wall
    ...
    # Description: To understand the direction of the building
